	...
	if node.get_parent() == root:
		...
		if not scene_path.is_empty():
			if _spawn_list.is_empty() or _spawn_list.has(scene_path):
				_client_recieve_node_add.rpc(node.get_path(), scene_path, node.get_multiplayer_authority())
...
func _client_recieve_node_add(node_path: NodePath, scene_path: String, mp_authority: int) -> void:
